/*****************************************************
     ...
     libioP.h - ANSI-C++ library: iostreams
 ----------------------------------------------------
...
#define _IO_JUMPS_V(THIS) ((struct _IO_FILE_plus*)(((char*)(THIS))-sizeof(void*)))->vtable
#endif
...
#ifndef _VIRTUAL_IMPLEMENTATION 
#define JUMP_FIELD(TYPE, NAME) struct { short delta1, delta2; TYPE pfn; } NAME
#define JUMP_INIT(NAME, VALUE) {0, 0, VALUE}
#else

/* HI-CROSS+ lets you configure the size of the delta field of the virtual table
with the option -Tvtd<No>
*/
#if defined(__VTAB_DELTA_IS_8BIT__)
  /* -Tvtd1   => 8 bit */
  #define JUMP_FIELD(TYPE, NAME) struct { TYPE pfn; Byte delta; } NAME
#elif defined(__VTAB_DELTA_IS_16BIT__)
  /* -Tvtd2   =>  16 bit */
  #define JUMP_FIELD(TYPE, NAME) struct { TYPE pfn; Word delta; } NAME
#elif defined(__VTAB_DELTA_IS_32BIT__)
  /* -Tvtd3   =>  32 bit */
  #define JUMP_FIELD(TYPE, NAME) struct { TYPE pfn; LWord delta; } NAME
#elif defined(__VTAB_DELTA_IS_64BIT__)
  /* -Tvtd4   =>  64 bit */
  #error "no virtual table with 64 bit delta entry available"
#elif !defined(__HIWARE__)
  #define JUMP_FIELD(TYPE, NAME) struct { TYPE pfn; Word delta; } NAME
#else
  #error "no virtual table delta size specified in compiler"
#endif


#define JUMP_INIT(NAME, VALUE) {VALUE, 0}
#endif
#ifndef _VIRTUAL_IMPLEMENTATION 
#define JUMP0(FUNC, THIS) _IO_JUMPS(THIS)->FUNC.pfn(THIS)
#define JUMP1(FUNC, THIS, X1) _IO_JUMPS(THIS)->FUNC.pfn(THIS, X1)
#else
#define JUMP0(FUNC, THIS) _IO_JUMPS_V(THIS)->FUNC.pfn(THIS)
#define JUMP1(FUNC, THIS, X1) _IO_JUMPS_V(THIS)->FUNC.pfn(THIS, X1)
#endif
...
struct _IO_jump_t { /* vtable for streambuf */
#ifndef _VIRTUAL_IMPLEMENTATION
    JUMP_FIELD(_G_size_t, __dummy);  // without => link error (procedure tag)
#endif    
#ifdef _G_USING_THUNKS
    JUMP_FIELD(_G_size_t, __dummy2);
#endif
    JUMP_FIELD(_IO_finish_t, __finish);   // virtual Dtor
...
struct _IO_FILE_plus {
  ...
  #ifndef _VIRTUAL_IMPLEMENTATION
    _IO_FILE file;
#if defined(__USER_DEF_QUALIS__) && defined(__CONST_PTR_SIZE_2__)
    struct _IO_jump_t *far vtable;
#else
    const struct _IO_jump_t *far vtable;
#endif
  #else
#if defined(__USER_DEF_QUALIS__) && defined(__CONST_PTR_SIZE_2__)
    struct _IO_jump_t *far vtable; // vt_streambuf;
#else
    const struct _IO_jump_t *far vtable; // vt_streambuf;
#endif
    _IO_FILE file;
  #endif
#else
  ...
  #else
#if defined(__USER_DEF_QUALIS__) && defined(__CONST_PTR_SIZE_2__)
    struct _IO_jump_t *vtable; // vt_streambuf;
#else
    const struct _IO_jump_t *vtable; // vt_streambuf;
#endif
    _IO_FILE file;
  #endif
#endif
...
extern unsigned _IO_adjust_column __P((unsigned, const char*_LIBDEF_FAR_CPP_PTR , int));
...
extern int _IO_do_write __P((_IO_FILE*, const char*_LIBDEF_FAR_CPP_PTR, _IO_size_t));
...
extern _IO_FILE* _IO_file_fopen __P((_IO_FILE*, const char*_LIBDEF_FAR_CPP_PTR, const char*_LIBDEF_FAR_CPP_PTR));
...
extern _IO_FILE* _IO_proc_open __P((_IO_FILE*, const char*_LIBDEF_FAR_CPP_PTR, const char*_LIBDEF_FAR_CPP_PTR ));
...
extern void _IO_str_init_readonly __P((_IO_FILE *, const char*_LIBDEF_FAR_CPP_PTR, int));
extern _IO_ssize_t _IO_str_count __P ((_IO_FILE*));

extern _IO_size_t _IO_getline __P((_IO_FILE*,char*,_IO_size_t,int,int));
extern _IO_ssize_t _IO_getdelim __P((char**, _IO_size_t*, int, _IO_FILE*));
extern double _IO_strtod __P((const char*_LIBDEF_FAR_CPP_PTR , char **));
...
#else
int _IO_outfloat __P((double __value, _IO_FILE *__sb, int __type,
         int __width, int __precision, int __flags,
         int __sign_mode, int __fill)) {exit(1); return 1;}
#endif
